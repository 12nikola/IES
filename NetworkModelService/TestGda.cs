	...
	{			
		...
		public ResourceDescription GetValues(long globalId)
		{
			...
			try
			{
                ...
                rd = GdaQueryProxy.GetValues(globalId, properties);  // server za taj gid se pozove i nazad se dobije Resurcedesc
		...
		public List<long> GetExtentValues(ModelCode modelCode)
		{
			...
			{				
                ...
                List<ModelCode> properties = modelResourcesDesc.GetAllPropertyIds(modelCode); //za taj model kod vratim sve propertije

                iteratorId = GdaQueryProxy.GetExtentValues(modelCode, properties); //dobijem iterator id,identifikator resursa
		...
		public List<long> GetRelatedValues(long sourceGlobalId, Association association)
		{
			...
			{						
                ...
                //Association association = new Association(referenca/lista referenci, type/0 po kome hocu da filtriram);
        ...
        public IEnumerable<long> GetOTValues()
        {
            return this.GetExtentValues(ModelCode.OUTAGESCHEDULE);
        }

        public IEnumerable<long> GetSWValues()
        {
            return this.GetRelatedValues(this.GetOTValues().Min(), new Association() { PropertyId = ModelCode.OUTAGESCHEDULE_SWITCHINGOPERATIONS, Type = ModelCode.SWITCHINGOPERATION });
        }
